...
export async function GET(_request: Request, { params }: RouteParams) {
  ...
  const url = `${BACKEND_URL}/uploads/${targetPath}`;

  try {
    const response = await fetch(url, { cache: "no-store" });

    if (!response.ok) {
      // Fallback: also try fetching from /media/
      const fallbackUrl = `${BACKEND_URL}/media/${targetPath}`;
      ...
      console.error(`[UploadsProxy] Failed to fetch ${url} - status: ${response.status}`);
    ...
    console.error(`[UploadsProxy] Fetch error for ${url}:`, err);
